...
def chunk_tags(tag_list, max_tags_per_chunk=100):
    # Split tags into smaller chunks to avoid exceeding the model's token limit
...
def generate_tags_with_llm(lista_path, cleaned_tags_path):
    ...
    for file_path in file_paths:
        if os.path.exists(file_path):
            ...
            # Remove duplicates from the final tags list
